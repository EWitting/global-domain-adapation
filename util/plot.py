...
def plot_target_acc_box(results: pd.DataFrame, name: str = None) -> None:
    ...
    title = "Target Domain Performance"
    if name:
        title = f"{name} - {title}"
    plt.title(title)
    plt.ylabel("Accuracy (%)")
...
def plot_adaptation(results: pd.DataFrame, name: str = None) -> None:
    """
...
def plot_relative_adaptation(df: pd.DataFrame, name: str = None) -> None:
    ...
    title = "Global versus target adaptation ratio"
    if name:
        title = f"{name} - {title}"
    plt.title(title)
