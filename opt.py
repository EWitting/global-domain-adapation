...
def objective(trial):
    ...
    #                     lambda_=trial.suggest_float('lambda', 0.0, 0.4),
    #                     gamma=trial.suggest_float('gamma', 3.5, 6),
    #                     metrics=["acc"], random_state=0)
    # fit_params = dict(epochs=25,
    #                   batch_size=16,
    #                   verbose=0)
    # acc = batch_eval_single(store_path, MDD, model_params, fit_params, 's', 'g')
    model_params = dict(input_dim=5, encoder_dim=3,
                        aux_classifier_weight=trial.suggest_float('aux_class_weight', 0.0, 5.0),
                        mmd_weight=trial.suggest_float('mmd_weight', 0.0, 2.0),
                        mmd_beta=trial.suggest_float('mmd_beta', 0.0, 5.0))
    fit_params = dict(epochs=15,
                      batch_size=16,
                      verbose=0)
    acc = batch_eval_single(store_path, Autoencoder, model_params, fit_params, 's', target_domain)
    return np.mean(acc)
